...
export default function InvestigationWorkspace({
  ...
  const sourceSpanStart = parseSearchOffset(searchParams.get("searchSpanStart"));
  const sourceSpanEnd = parseSearchOffset(searchParams.get("searchSpanEnd"));
  const sourceSpanText = searchParams.get("searchSpanText") ?? "";
  ...
  return (
    <section aria-label="Investigation workspace" className="space-y-6">
      ...
      {selectedView === "evidence" ? (
        <EvidenceView workspace={workspace} onOpenSource={(id, span) => updateParams({ source: id, searchSpanStart: span ? String(span.start) : null, searchSpanEnd: span ? String(span.end) : null, searchSpanText: span?.text ?? null })} />
      ) : null}
      {selectedView === "narrative" ? (
        <NarrativeView
          ...
          onOpenAudit={() => updateParams({ view: "method", source: null })}
        />
      ) : null}
      {selectedView === "method" ? (
      ...
      <SourceDrawer source={source} span={sourceSpanStart !== null && sourceSpanEnd !== null ? { start: sourceSpanStart, end: sourceSpanEnd, text: sourceSpanText } : null} onClose={() => updateParams({ source: null, searchSpanStart: null, searchSpanEnd: null, searchSpanText: null })} />
...
function WorkspaceNavigation({
  ...
  const methodTabRef = useRef<HTMLButtonElement>(null);
  useEffect(() => {
    if (activeView === "method") methodTabRef.current?.focus();
  }, [activeView]);
  return (
    <div className="sticky top-3 z-20 rounded-[1.4rem] border border-[var(--border)] bg-white/90 p-2 shadow-[0_18px_45px_-35px_rgba(19,35,58,0.5)] backdrop-blur-xl">
      <div aria-label="Investigation views" role="tablist" className="flex flex-wrap gap-1">
        {VIEWS.map(([id, label]) => (
          <button
            key={id}
            ref={id === "method" ? methodTabRef : undefined}
            type="button"
            role="tab"
...
function EvidenceView({ workspace, onOpenSource }: { workspace: LiveInvestigationWorkspace; onOpenSource: (id: string, span?: { start: number; end: number; text: string }) => void }) {
  const [searchParams, setSearchParams] = useSearchParams();
  ...
  return <div role="tabpanel" className="space-y-6">
    ...
    <B5EvidenceSearch investigationId={workspace.investigation_id} sourceTypes={types} onOpenSource={onOpenSource} />
...
function parseSearchOffset(value: string | null): number | null { if (value === null || !/^\d+$/.test(value)) return null; const parsed = Number(value); return Number.isSafeInteger(parsed) ? parsed : null; }
