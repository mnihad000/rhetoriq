...
The frontend is a **React + TypeScript** single-page application built with Vite. It connects to the backend API via REST and WebSocket for live updates. The core feature is the spread graph â€” an interactive visualization of how a narrative traveled from its origin to mainstream adoption.
...
â””â”€â”€ src/
    ...
    â”œâ”€â”€ types/
    â”‚   â”œâ”€â”€ investigation.ts      # Investigation and report types
    â”‚   â”œâ”€â”€ narrative.ts          # Narrative and anomaly types
    â”‚   â””â”€â”€ graph.ts              # Graph node and edge types
    â”œâ”€â”€ api/
    â”‚   â”œâ”€â”€ client.ts             # Axios instance with auth headers
    â”‚   â”œâ”€â”€ investigations.ts     # Investigation API calls
    â”‚   â”œâ”€â”€ narratives.ts         # Narrative API calls
    â”‚   â”œâ”€â”€ search.ts             # Search API calls
    â”‚   â””â”€â”€ graph.ts              # Graph API calls
    â”œâ”€â”€ hooks/
    â”‚   â”œâ”€â”€ useInvestigations.ts  # React Query hooks for investigations
    â”‚   â”œâ”€â”€ useNarratives.ts      # React Query hooks for narratives
    â”‚   â”œâ”€â”€ useWebSocket.ts       # WebSocket connection hook
    â”‚   â””â”€â”€ useSearch.ts          # Search hook with debounce
    â”œâ”€â”€ store/
    â”‚   â””â”€â”€ liveStore.ts          # Zustand store for live feed state
    â”œâ”€â”€ pages/
    â”‚   â”œâ”€â”€ Dashboard.tsx         # Main dashboard â€” live feed + trending
    â”‚   â”œâ”€â”€ Investigation.tsx     # Single investigation report page
    â”‚   â”œâ”€â”€ Search.tsx            # Phrase search page
    â”‚   â””â”€â”€ NotFound.tsx          # 404 page
    â”œâ”€â”€ components/
    â”‚   â”œâ”€â”€ layout/
    â”‚   â”‚   â”œâ”€â”€ Sidebar.tsx       # Navigation sidebar
    â”‚   â”‚   â”œâ”€â”€ Header.tsx        # Top header with search bar
    â”‚   â”‚   â””â”€â”€ Layout.tsx        # Root layout wrapper
    â”‚   â”œâ”€â”€ dashboard/
    â”‚   â”‚   â”œâ”€â”€ LiveFeed.tsx      # Real-time narrative detection feed
    â”‚   â”‚   â”œâ”€â”€ TrendingCard.tsx  # Single trending narrative card
    â”‚   â”‚   â””â”€â”€ TimelineChart.tsx # Recharts time-series of detections
    â”‚   â”œâ”€â”€ investigation/
    â”‚   â”‚   â”œâ”€â”€ ReportViewer.tsx  # Renders configured LLM markdown report
    â”‚   â”‚   â”œâ”€â”€ SpreadTimeline.tsx # Visual spread path timeline
    â”‚   â”‚   â”œâ”€â”€ AmplifierList.tsx  # Key amplifiers ranked list
    â”‚   â”‚   â””â”€â”€ PatternBadge.tsx   # Grassroots/Top-down/etc badge
    â”‚   â”œâ”€â”€ graph/
    â”‚   â”‚   â”œâ”€â”€ SpreadGraph.tsx   # Sigma.js graph visualization
    ...
    â””â”€â”€ utils/
        â”œâ”€â”€ dates.ts              # Date formatting helpers
        â”œâ”€â”€ markdown.ts           # Markdown rendering config
        â””â”€â”€ graphLayout.ts        # Sigma.js layout helpers
```
...
1. **Live Feed** â€” real-time stream of detected anomalies and investigations powered by WebSocket
2. **Trending** â€” top 10 narratives by spike magnitude in the last 24 hours
3. **Timeline Chart** â€” Recharts time-series showing detection frequency over the last 7 days
...
export default function Dashboard() {
  ...
  return (
    <div className="grid grid-cols-12 gap-6 p-6">
      ...
      {/* Live Feed â€” left column */}
      <div className="col-span-4">
        <h2 className="text-lg font-semibold mb-4">Live Feed</h2>
        <LiveFeed />
      </div>

      {/* Trending + Timeline â€” right columns */}
...
1. **Report Viewer** â€” renders the configured LLM markdown report
2. **Spread Timeline** â€” horizontal visual timeline of the spread path stages
3. **Spread Graph** â€” interactive Sigma.js graph visualization
4. **Amplifier List** â€” ranked list of key amplifier nodes
  ...
  useEffect(() => {
    ...
    graphData.nodes.forEach((node) => {
      graph.addNode(node.id, {
        ...
        x: Math.random(),  // Initial random position â€” layout algorithm takes over
        y: Math.random(),
...
Renders the configured LLM generated markdown report with syntax highlighting and clean typography.
...
export function useInvestigation(id: string) {
  return useQuery({
    ...
    staleTime: 60 * 60 * 1000,  // 1 hour â€” reports don't change
    enabled: !!id,
  });
}
```
...
Only live feed data lives in Zustand â€” everything else is React Query server state.
