...
`further_dev` release branch must receive the foundation publisher-role ARN in
the `ecr-release` environment, which already permits only `further_dev`. An
approved GitHub Actions run on `further_dev` must then build and push all four
Linux/AMD64 images from one explicit source SHA and verify their registry
digests before application deployment. Do not force Docker,
